...
var incrementButton = document.getElementById('increaseTempButton');
var decrementButton = document.getElementById('decreaseTempButton');
var seasonalChange = document.getElementById('seasonalChange');
...
var resetButton = document.getElementById('resetBtn'); // const selectSkies = document.getElementById('skies');
...
var incrementTemp = function incrementTemp() {
  state.temp += 1;
  updateTemp();
  alterTextColor();
};

var decrementTemp = function decrementTemp() {
  state.temp -= 1;
  updateTemp();
  alterTextColor();
};

var updateCity = function updateCity() {
  var currentCity = document.getElementById('searchCity').value;
...
var changeSky = function changeSky() {
  ...
  if (selection === 'clearDay') {
    ...
    seasonalChange.textContent = '🔆'; // skyContent = 'linear-gradient(to bottom, '#daf4ff', '#d1eaff', '#bfd7ff')'
...
document.getElementById('skies').addEventListener('change', changeSky); // change the sky
...
var registerEventHandlers = function registerEventHandlers() {
  ...
  currentTempButton.addEventListener('click', findLatitudeAndLongitude); //   selectSkies.addEventListener('change', changeSky);
};

document.addEventListener('DOMContentLoaded', registerEventHandlers);

var alterTextColor = function alterTextColor() {
  if (state.temp > 95) {
...
resetButton.addEventListener('click', refreshPage); // const setIcons = () => {
...
var findLatitudeAndLongitude = function findLatitudeAndLongitude() {
  ...
  _axios.default.get('https://weather-report-proxy-server.herokuapp.com/location', {
    ...
    longitude = response.data[0].lon; // make the next API call here!
...
var findTemperature = function findTemperature(latitude, longitude) {
  _axios.default.get('https://weather-report-proxy-server.herokuapp.com/weather', {
    ...
    var kelvin = response.data['current']['temp'];
